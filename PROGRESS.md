...
- **🏛️ DESIGN SENSE THREAD (~25 min, once a week, from W3 — NOT a phase, a drip):**
  ...
  **spaced repetition on my own code, wearing a senior hat.**
  ...
  - Attachment map: **W3** SOLID/single-responsibility · **W6** who owns state · **W7** layering + dependency direction ·
  ...
  - **🚪 The check that this worked:** by **W10** — when Track C's design reps start — I can hold a
    mobile-flavoured design conversation using only the **4 drips that landed through W9**:
    SOLID (W3) · who owns state (W6) · layering + dependency direction (W7) · client↔server
    boundary + DI (W8). **API contracts land W10–12, i.e. *during* the rep window, not before it** —
    so my first October reps are honestly app-architecture reps, and the API/backend flavour arrives
    a fortnight in. If even the 4 don't hold, the drip was too thin and W21 has to carry more.
...
- ⬜ **W6 (Sep 5–11):** State & recomposition, user input, LazyColumn, Navigation, multi-screen → **📦 Project: Notes app** · 🏛️ **design drip:** *where does state live, and who owns it?* — state hoisting as an architecture decision, not a Compose trick
